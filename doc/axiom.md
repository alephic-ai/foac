...
`query` posts an APL query to `/v1/datasets/_apl` and prints Axiom's raw
result. The default `tabular` format returns `tables[]` with `fields` and
columnar `columns`; `--legacy` returns one object per event under
`matches[].data`, which is easier to read for small result sets. Time
ranges go in the query (`where _time > ago(1h)`) or in `--start-time` /
`--end-time` as RFC 3339. For cursor paging, sort by `_time`, then pass
`status.maxCursor` (ascending) or `status.minCursor` (descending) back
