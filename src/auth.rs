...
pub fn run(
    ...
    match cmd.command {
        ...
        AuthCmd::Jira(cmd) => {
            crate::atlassian::run_atlassian(Provider::Jira, cmd.command, &store, format, instance)
        }
        AuthCmd::Confluence(cmd) => crate::atlassian::run_atlassian(
            Provider::Confluence,
            cmd.command,
            &store,
            format,
...
pub(crate) fn status_key(provider: Provider, instance: &str) -> String {
    if instance == DEFAULT_INSTANCE {
...
pub(crate) fn login_report(provider: Provider, instance: &str, account: Value) -> Value {
    nest(
        provider,
        instance,
        json!({
...
pub(crate) fn status_summary(provider: Provider, body: &Value) -> String {
...
fn provider_status(provider: Provider, store: &dyn SecretStore, instance: &str) -> Value {
    ...
    if matches!(provider, Provider::Jira | Provider::Confluence) {
        return match crate::atlassian::resolve_atlassian(
            crate::atlassian::atlassian_environment(),
            store,
            instance,
        ) {
            Ok(resolved) => credential_status(
                ...
                |token| {
                    crate::atlassian::atlassian_account(
                        provider,
...
pub(crate) fn environment(variable: &str) -> Option<String> {
...
pub(crate) fn read_token() -> Result<String, Box<dyn std::error::Error>> {
    if std::io::stdin().is_terminal() {
...
pub(crate) mod tests {
    ...
    pub(crate) struct MemoryStore {
        pub(crate) credentials: RefCell<HashMap<(crate::provider::Credential, String), String>>,
        pub(crate) get_error: Option<String>,
    }
